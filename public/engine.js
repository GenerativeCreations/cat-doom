...
const texHairball = makeTex(g => { g.clearRect(0, 0, TEX, TEX); g.fillStyle = '#e8dcc4'; g.beginPath(); g.arc(32, 32, 19, 0, 6.3); g.fill(); g.fillStyle = '#9a7a5a'; for (let i = 0; i < 40; i++) { const a = i * 0.9, r = 12 + (i % 5) * 2; g.beginPath(); g.arc(32 + Math.cos(a) * r * 0.5, 32 + Math.sin(a) * r * 0.5, 6, 0, 6.3); g.fill(); } g.fillStyle = '#c8a888'; for (let i = 0; i < 14; i++) g.fillRect(22 + (i * 7) % 20, 22 + (i * 11) % 20, 3, 3); g.strokeStyle = '#3a2a1a'; g.lineWidth = 1.5; for (let i = 0; i < 6; i++) { const a = i * 1.05; g.beginPath(); g.moveTo(32 + Math.cos(a) * 14, 32 + Math.sin(a) * 14); g.lineTo(32 + Math.cos(a) * 21, 32 + Math.sin(a) * 21); g.stroke(); } });
...
function render() {
  ...
  for (const s of G.shots) sprites.push({ x: s.x, y: s.y, d: Math.hypot(s.x - p.x, s.y - p.y), img: texHairball, scale: 0.3, bob: Math.sin(G.t * 25 + s.x * 3) * 1.5, z: 0.32, alpha: 1 });
